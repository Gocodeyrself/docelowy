...
{if isset($trackmultiplecarriers) && $trackmultiplecarriers}
	{foreach from=Trackmultiplecarriers::getTrackMultipleCarriersShipping($order->id) item=line}
		<tr id="trackmultiplecarriers_{$line.id_tracknum|intval}">
			...
			<td>
				{if $line.can_edit}
					<a href="{$line.id_tracknum|intval}" class="btn btn-primary addons4presta_trackmultiplecarriers_add_btn" rel="1" data-toggle="modal" data-target="#trackMultipleCarriersModal">{l s='Edit' mod='sensbitgls'}</a>
					<a href="{$line.id_tracknum|intval}" class="btn btn-default tmc-delete" rel="2">{l s='Delete' mod='sensbitgls'}</a>
				{/if}
			</td>
		</tr>
	{/foreach}
