...
<tr id='package_{$uniq}' class='package' data-id='{$uniq}'>
	...
	<td>
		<input class='param form-control' name="email" type="email" value="{$data.email}" placeholder="{l s='Email' mod='sensbitgls'}" />
		<input class='param form-control' name="phone" type="text" value="{if empty($data.phone_mobile)}{$data.phone}{else}{$data.phone_mobile}{/if}" placeholder="{l s='Phone' mod='sensbitgls'}"/>
	</td>
	<td>
		{if $data.is_point}
			<input style='width: 100%' type='text' name='id_point' value="{$data.id_point}" class='param package_{$uniq}_id_point' placeholder="{l s='ID punktu odbioru' mod='sensbitgls'}"/>
		...
		<div class='address' {if $data.is_point}style="display:none"{/if}>
			...
			<button class="btn btn-default btn-xs edit-address tip" title="{l s='Edit address' mod='sensbitgls'}"><i class="icon-edit"></i></button>
		</div>
		<div class='address-edit' style="display:none; max-width:230px;">
			<input type='text' class='param' value="{$data.company}" name='company' placeholder='{l s='Company' mod='sensbitgls'}'/>
			<div class='row'>
				<div class='col-sm-6'>
					<input type='text' class='param' value="{$data.firstname}" name='firstname' placeholder='{l s='Firstname' mod='sensbitgls'}'/>
				</div>
				<div class='col-sm-6'>
					<input type='text' class='param' value="{$data.lastname}"  name='lastname' placeholder='{l s='Lastname' mod='sensbitgls'}'/>
				</div>
			</div>
			<div class='row'>
				<div class='col-sm-6'>
					<input type='text' class='param' value="{$data.address1}"  name='street' placeholder='{l s='Street' mod='sensbitgls'}'/>
				</div>
				<div class='col-sm-6'>
					<input type='text' class='param' value="{$data.address2}"  name='building_number' placeholder='{l s='Building number' mod='sensbitgls'}'/>
				</div>
			</div>
			<div class='row'>
				<div class='col-sm-4'>
					<input type='text' class='param' value="{$data.postcode}"  name='postcode' placeholder='{l s='Postcode' mod='sensbitgls'}'/>
				</div>
				<div class='col-sm-4'>
					<input type='text' class='param' value="{$data.city}"  name='city' placeholder='{l s='City' mod='sensbitgls'}'/>
				</div>
				<div class='col-sm-4'>
					<input type='text' class='param' value="{$data.country_iso_code}"  name='country_iso_code' placeholder='{l s='Country code' mod='sensbitgls'}'/>
				</div>
			</div>

			<button class="btn btn-success btn-xs save-address tip" title="{l s='Save address' mod='sensbitgls'}"><i class="icon-check"></i></button>
	...
	<td>
		{if isset($data.options.cod)}
			<div class="input-group">
				<span class="input-group-addon"><input class='param' placeholder="{l s='Is COD' mod='sensbitgls'}" name="cod" type="checkbox"{if $data.options.cod} checked='checked'{/if}/></span>
				<input class='param' style="width:70px" placeholder="{l s='COD value' mod='sensbitgls'}" name="cod_value" type="text" value="{if empty($data.options.cod_value)}{$data.total_paid_tax_incl|round:2}{else}{$data.options.cod_value}{/if}"/>
				<input class='param' style="width:40px;text-align: center" placeholder="{l s='COD Currency' mod='sensbitgls'}" name="cod_currency" type="text" value="{$data.currency}"/>
			</div>
		{else}
			-
		{/if}
	...
	<td>
		<input class='param' name="reference" type="text" value="{$data.custom_reference}" placeholder="{l s='Reference' mod='sensbitgls'}"/>
	</td>
	<td>
