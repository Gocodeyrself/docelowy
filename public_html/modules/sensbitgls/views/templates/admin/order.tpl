...
{capture name='content'}
	<form class='packages-ready-form messages-container'{if empty($data.shipments)} style="display:none"{/if}>
		<h4>{l s='Packages created for order' mod='sensbitgls'} {$data.order->reference}</h4>
		<table class='table'>
			<thead>
				<tr>
					<th></th>
					<th>{l s='Service' mod='sensbitgls'}</th>
					<th>{l s='Tracking number' mod='sensbitgls'}</th>
					<th>{l s='Status przesyłki' mod='sensbitgls'}</th>
					<th>{l s='Pobranie' mod='sensbitgls'}</th>
					<th>{l s='Create date' mod='sensbitgls'}</th>
					<th>{l s='Created by' mod='sensbitgls'}</th>
					<th></th>
				</tr>
			</thead>
		...
		<div class="packages-completed-actions">
			<button class="btn btn-default print-labels">{l s='Print labels' mod='sensbitgls'}</button>
			<button class="btn btn-default delete-shipments">{l s='Cancel shipments' mod='sensbitgls'}</button>
	...
	{else}
		{if !empty($data.templates)}
			...
			{foreach $data.templates as $template}
				<a href='' class='button btn btn-default btn-xs sensbitgls-service' data-id='{$template.id_template}'>{$template.name} {SensbitGlsService::getServiceLogo($template.service)|unescape}</a>
			{/foreach}
		{else}
		...
		<form class='packages-form messages-container' style="display:none">
			{*<h3>{l s='Create new package for order ' mod='sensbitgls'} {$data.order->reference}</h3>*}
			<table class='table'>
				<thead>
					<tr>	
						<th></th>
			...
			<div style="margin-top:20px">
				<button class="btn btn-warning prepare-packs">{l s='Create' mod='sensbitgls'}</button>
			</div>
		</form>

		<script>
			{if !SensbitGlsTools::isBootstrap()}
			$('.sensbitgls').eq(0).insertAfter($('.sensbitgls').eq(0).parent());
			{/if}
			sensbitgls.setOptions({
				id_order: {$data.order->id|intval},
				ajax_url_packages: '{$link->getAdminLink('AdminSensbitGlsPackage')}',
...
<div class="sensbitgls{if $data.hide_global_templates} hide_global_templates{/if}{if empty($data.templates) && $data.hide_panel_if_no_templates} hide_no_templates{/if}">
	{if $data.bootstrap}
		<div class="panel">
			<div class="panel-heading">
				<i class="icon-truck"></i> Wysyłka z GLS <a href="{$data.module_link}"><i class="icon-cogs"></i></a> <a href="#" class="switch_no_templates s">Pokaż</a><a href="#" class="switch_no_templates h">Ukryj</a>
			</div>
