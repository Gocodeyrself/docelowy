  ...
  var l = {
        ...
        void 0 !== s && (s(function() {
          console.log(r.text_console, "font-size:18px;color:#e40037;"), r.hide_map || (s("." + r.module + "-map-container").remove(), s("body").append("<div class='" + r.module + "-map-container'><div id='sensbitgls-easypack-map'></div></div>"), s("." + r.module + "-map-container").click(function(e) {
            ...
            s(o).filter(":visible").each(function() {
              var e = s(this);
              e.hasClass(".point-select") || e.val(t.id), s(o).parents("." + r.module).find("." + r.select2 + "-selection__rendered").text([t.id, t.name, t.street, t.postal_code, t.city, t.country].join(", ")), l.updatePoint(t.id, o)
            }), s("." + r.module + "-map-container").fadeOut(), r.map_is_open = !1
            ...
            r.point_required && r.point.length <= 0 ? (e.preventDefault(), s("." + r.module).addClass("error"), alert(r.text_choose_point), s(this).is(":checkbox") && (s(r.conf_btn).prop("checked", !1), void 0 !== s.uniform) && s.uniform.update(r.conf_btn), s("." + r.module + "-map-btn").trigger("click")) : s("." + r.module).removeClass("error")
    ...
    updatePoint: function(t, o) {
      s.ajax({
        ...
        success: function(e) {
          "0" === e ? (r.point = "", s(o).closest("." + r.module).find("." + r.select2 + "-selection__rendered").text(""), e = r.text_point_cod, s.prototype.fancybox ? s.fancybox.open([{
            type: "inline",
            autoScale: !0,
            minHeight: 30,
          ...
          }) : alert(e)) : (r.point = t, s("." + r.module).removeClass("error"))
        }
      })
    }
  };
