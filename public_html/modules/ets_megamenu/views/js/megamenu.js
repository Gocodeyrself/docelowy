...
$(function () {
  ...
  $(document).on(
    'mouseenter',
    '.ets_mm_megamenu .mm_menus_li.mm_has_sub',
    function (e) {
      if (
        ($(window).width() > 767 &&
          $('.ets_mm_megamenu:not(.changestatus)').length > 0) ||
        ($(window).width() > 1199 &&
          $('.ets_mm_megamenu.changestatus').length > 0)
      ) {
        var _mm_item_position = $(this).position().top + $(this).outerHeight();
        $(this)
          .find('.mm_columns_ul')
          .css('top', _mm_item_position + 'px');
      }
    }
  );
...
function autoChangeStatus() {
  ...
  console.log(width_li_menu, width_ul_menu);
  if (width_li_menu > width_ul_menu + 5 && document.body.clientWidth <= 767) {
