...
class AdminGformrequestController extends ModuleAdminController
{
                    ...
                    if($email !=''){
                        if(!Validate::isCleanHtml($gformbuilderpro_reply)){
                            ...
                            if($requestObj->jsonrequest !=''){
                                $jsonrequest = json_decode($requestObj->jsonrequest,true);
                                if($jsonrequest){
                            ...
                            foreach($emails as $_email){
                                ...
                                if($_email !='' && Validate::isEmail($_email)){
                                    ...
                                    $this->parseEmailReplyAndTpl((int)$requestObj->id_gformbuilderpro,$id_lang,$id_shop, $gformbuilderpro_reply,$subject);
                                    Mail::Send(
                                        (int)$id_lang,
    ...
    public function renderView(){
        ...
        if($idrequest > 0){
            ...
            //Tung fix missing message respon 14/4/2023
            $email_template = $formObj->replyemailtemplate;
            $this->tpl_view_vars['email_template'] =  $email_template;
    ...
    public function parseEmailReplyAndTpl($id_form,$lang,$id_shop=null,$content_reply,$subject){
        $formObj = new gformbuilderproModel((int)$id_form);
      
        if($formObj->id_gformbuilderpro == (int)$id_form){
            $lang=new Language($lang);
            $this->parseEmail($lang->id,'form_'.(int)$formObj->id_gformbuilderpro.'_'.(int)$id_shop,$subject,$content_reply);
        }

    }
    public function parseEmail($lang,$filename,$subject,$content_reply){
        // print_r('<pre>');print_r($lang);die;
       $lang_iso=Language::getIsoById($lang);
     
        $isps17 = version_compare(_PS_VERSION_, '1.7', '>=');
        Context::getContext()->smarty->assign(array('isps17' => $isps17));
        $mails_dir = _PS_MODULE_DIR_.'gformbuilderpro/mails/';
        if (!is_dir($mails_dir.$lang_iso.'/')){
            @mkdir($mails_dir.$lang_iso.'/', 0755);
        }
        if(!file_exists($mails_dir.$lang['iso_code'].'/index.php'))
            @copy(_PS_MODULE_DIR_.'gformbuilderpro/index.php',$mails_dir.$lang_iso.'/index.php');
        
        /* reply template */
        $file = $mails_dir.$lang_iso.'/reply_'.$filename.'.html';
       
        $fields_value = array();
        $tpl = _PS_MODULE_DIR_.'gformbuilderpro/views/templates/front/emailbase.tpl';
        $emailcontent = $this->fixMce($content_reply);   
        $fields_value = array(
            'subject'=>$subject,
        ...
        fclose($handle);
        $file = $mails_dir.$lang_iso.'/reply_'.$filename.'.txt';
       
        $handle = fopen($file, 'w+');
        fwrite($handle, strip_tags($content_reply));
        fclose($handle);
