    ...
    public function postProcess()
	{
	   ...
	   elseif(Tools::isSubmit('submitGexport_form'))
       {
	       ...
	       $query = Tools::getValue('q', false);
            ...
            $combins  = json_decode(Tools::getValue('combins'), true);
            ...
            $results  = array();
            if($products) {
                $combins = json_decode($combins, true);
        ...
        if($id_gformbuilderpro>0){
            ...
            if($fields){
                ...
                if($_fielddatas){
                    foreach($_fielddatas as $fielddata){
                        ...
                        $fielddatas[(int)$fielddata['id_gformbuilderprofields']]['condition_listoptions'] = $fielddata['condition_listoptions'] !='' ? json_decode($fielddata['condition_listoptions'], true) : array();
                        $fielddatas[(int)$fielddata['id_gformbuilderprofields']]['number_listcondition'] = $fielddata['condition_listoptions'] !='' ? count(json_decode($fielddata['condition_listoptions'], true)) : 0;
           ...
           $this->fields_value = array(
            ...
            if($condition_configs !=''){
                $this->fields_value['condition_configs'] = $this->module->htmlEntityDecode(json_decode($condition_configs,true));
            }
            foreach($languages as $language){
        ...
        */
        ...
        $this->fields_value['config_link'] = $this->context->link->getAdminLink('AdminGformconfig');
            ...
            Context::getContext()->smarty->assign(array(
                    ...
                    'Currencies'=> json_encode(Currency::getCurrencies(false, true, true)),
                    'id_currency_default' => $this->context->currency->id,
                ));
            if(version_compare(_PS_VERSION_,'1.6') == -1){
                Context::getContext()->smarty->assign(array('allfieldstype'=>json_encode($allfieldstype)));
            }
