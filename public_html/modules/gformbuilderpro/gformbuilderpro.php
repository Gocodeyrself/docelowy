...
class Gformbuilderpro extends Module
{
    ...
    public function install()
    {
        ...
        else $res &= Configuration::updateValue('GF_PRODUCT_TYPE',ImageType::getFormatedName('home'));
        /* end */
    ...
    public function getContent()
	{
        if (Tools::isSubmit('duplicateField')){
            ...
            if($id_gformbuilderprofield > 0){
                ...
                if (isset($result['object']->condition_listoptions)) {
                    $result['object']->condition_listoptions = json_decode($result['object']->condition_listoptions, true);
                }
            }
	   ...
	   elseif (Tools::isSubmit('addThumb')){
           ...
           $fieldObj->condition_listoptions = json_encode(Tools::getValue('listoptions', array()));
            ...
            if ($fieldObj->type == 'wholesale') {
                ...
                if ($gform_idproducts) {
                    foreach($gform_idproducts as $id_product) {
                        ...
                        $option_whosales[$id_product]['gform_products_voucher'] = isset($gform_products_voucher[$id_product]) ? json_decode($gform_products_voucher[$id_product], true) : array();
                    }
                }
                $fieldObj->extra = json_encode($option_whosales);
            }
           ...
           die(json_encode(array('id_gformbuilderprofields'=>0)));
        }else
                    ...
                    if($fieldsData){
                        foreach($fieldsData as $field){
                            if($field['type'] == 'product'){
                                ...
                                if ($field['extra']) {
                                    $productDatas = array();
                                    $wholesale_productids = json_decode($field['extra'], true);
                            ...
                            $Conditions[(int)$field['id_gformbuilderprofields']] = array(
                                ...
                                'condition_listoptions' => $field['condition_listoptions'] !='' ? json_decode($field['condition_listoptions'], true) : array(),
                            );
                        }
                    }

                    $this->context->smarty->assign(array('Conditions'=>json_encode($Conditions)));
                    if($isps17){
   ...
   public function parseEmailAndTpl($id_form,$lang,$id_shops=null){
    ...
    public function getMailChimpList()
    {
        ...
        if($apiKey !=''){
            ...
            if($response){
                $mailchimp_lists = json_decode($response,true);
    ...
    public function getKlaviyoList(){
        ...
        if($apiKey !=''){
            ...
            if($response){
                $klaviyo_lists = json_decode($response,true);
