...
abstract class AbstractCategoryDataFactory implements GridDataFactoryInterface
{
    ...
    protected function modifyRecords(array $records): array
    {
        foreach ($records as $key => $record) {
            // Check if description is not null before applying stripslashes
            $description = $record['description'] ?? '';
            $records[$key]['description'] = mb_substr(strip_tags(stripslashes($description)), 0, self::DESCRIPTION_MAX_LENGTH);
        }
