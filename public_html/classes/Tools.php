...
';
    ...
    public static function getProductAccessories($id_product)
    {
        ...
        $query = 'SELECT p.reference, pl.name FROM `ps_accessory` a
            left join ps_product p on p.id_product = a.id_product_2
            left join ps_product_lang pl on pl.id_product = p.id_product
            where  pl.id_lang = 1 and a.id_product_1 = '.$id_product;
        return DB::getInstance()->executeS($query);
    }
    

    public static function changeLayout($type)
    {
        global $cookie;
        $cookie->__set('miniatures_layout', $type);
    }

    public static function isCmsMenu($cms){
        return Tools::checkIsCmsMenu($cms["id_cms_category"]);
    }

    public static function checkIsCmsMenu($id_cms_category){
        $cmsCategory = new CMSCategory($id_cms_category);
        if($cmsCategory->id_parent == 7 || $id_cms_category == 7){
            return true;
        }else if($cmsCategory->id_parent == 0){
            return false;
        }else{
            return Tools::checkIsCmsMenu($cmsCategory->id_parent);
        }
    }

    public static function getUnit($id_product, $id_feature = 42) {
        ...
        // Pobranie cech produktu
        $productFeatures = $product->getFrontFeatures(Context::getContext()->language->id);
        // Iteracja przez cechy produktu
        foreach ($productFeatures as $productFeature) {
            // Sprawdzenie, czy ID cechy pasuje do oczekiwanego ID
            if ($productFeature['id_feature'] == $id_feature) {
                // Pobranie wartości cechy
                $featureValue = new FeatureValue($productFeature['id_feature_value']);
    ...
    public static function getQuantityFromFeature($id_product){
        $sql = "SELECT fvl.value FROM `ps_feature_product` fp
        join `ps_feature_value_lang` fvl on  fvl.id_feature_value = fp.id_feature_value
        where id_feature = 52 and fvl.id_lang = 1 and fp.id_product =$id_product";
    ...
    public static function generateFlag($id_category, $id_product){
        $sql = "select * from ps_category_product where id_category = $id_category and id_product = $id_product";
    ...
    public static function generateCmsMenu($id_cms, $parentCategoryId = 7, $depth = 0) {
        $menuHtml = '';
    
        // Pobieranie podkategorii dla danej kategorii
        $subCategories = CMSCategory::getChildren($parentCategoryId, (int)Context::getContext()->language->id, true);
    
        // Pobieranie stron CMS dla danej kategorii
        $cmsPages = CMS::listCms((int)Context::getContext()->language->id, $parentCategoryId, true);
        
        if (!empty($subCategories) || !empty($cmsPages) || $id_cms==0) {
            if(!empty($subCategories)){
                foreach ($subCategories as $subCategory) {
                    $menuHtml .= "<div class='cms-menu-category' attr-depth='$depth'>";
                    $menuHtml .= '<h5 onclick="handleCmsCategory(event)">' . $subCategory['name'] . '<div class="icons-container"><i class="fas fa-chevron-right"></i><i class="fas fa-chevron-down"></i></i></h5>';
                    
                    $menuHtml .= Tools::generateCmsMenu($id_cms, $subCategory['id_cms_category'], $depth + 1);
                    $menuHtml .= "</div>";
                } 
            }
