...
class FrontControllerCore extends Controller
{
    ...
    public function init()
    {
        ...
        $cms = new CMS(6, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('headerSocialmedia', $cms->content);
        }
        
        $cms = new CMS(7, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('tilesCategoriesTwo', $cms->content);
        }
        
        $cms = new CMS(8, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('tilesCategories', $cms->content);
        }
        
        $cms = new CMS(9, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('footerRight', $cms->content);
        }
        
        $cms = new CMS(10, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('discoverCezos', $cms->content);
        }
        
        $cms = new CMS(11, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('consultant', $cms->content);
        }
        
        $cms = new CMS(12, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('yourPartner', $cms->content);
        }
        
        $cms = new CMS(13, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('footerLeft', $cms->content);
        }

        $cms = new CMS(23, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('footerCenterRight', $cms->content);
        }
        
        $cms = new CMS(24, intval($id_lang));
        if (Validate::isLoadedObject($cms) && $cms->active){
          $this->context->smarty->assign('totalFooter', $cms->content);
        }

        $store = new Store(6);
        $this->context->smarty->assign('store', $store);
        $opening_hours  = json_decode($store->hours[1]);
        $this->context->smarty->assign('opening_hours', $opening_hours[0][0]);

        global $cookie;
        $miniaturesLayout = isset($cookie->miniatures_layout) ? $cookie->miniatures_layout : 2;
        $this->context->smarty->assign('miniaturesLayout', $miniaturesLayout);
