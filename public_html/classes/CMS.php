...
class CMSCore extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'content' => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml', 'size' => 4999999999999],
        ],
    ];
    ...
    public static function listCms($idLang = null, $idBlock = false, $active = true)
    {
		...
		' . Shop::addSqlAssociation('cms', 'c') . '
		' . (($idBlock) ? 'JOIN ' . _DB_PREFIX_ . 'cms_category b ON (c.id_cms_category = b.id_cms_category)' : '') . '
		WHERE l.id_lang = ' . (int) $idLang . (($idBlock) ? ' AND b.id_cms_category = ' . (int) $idBlock : '') . ($active ? ' AND c.`active` = 1 ' : '') . '
		GROUP BY c.id_cms
		ORDER BY c.`position`');
    }
    ...
    public static function generateBlogImg($id_cms){
        $imgQuery = 'SELECT * FROM `'._DB_PREFIX_.'cms_img` WHERE id_cms = '.$id_cms;
