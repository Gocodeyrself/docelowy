...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function init()
    {
        ...
        if(isset($_GET["pdf"]) && $_GET["pdf"]=="true"){           
            ...
            $link = new Link();
            $url = $link->getProductLink($this->product);
            $qrcode = '<barcode code="'.$url.'" type="QR" class="barcode">';

            $id_product = $this->product->id;

            $photos = $this->product->getImages(Context::getContext()->language->id);
            $main_photo = reset($photos);
            $main_photo_url = _PS_BASE_URL_.'/'.$main_photo["id_image"].'-large_default/'.$this->product->link_rewrite.'.jpg';

   
            $description_short = str_replace("</li>","",$this->product->description_short);
            $description_short = str_replace("</ul>","",$description_short);
            $description_short = str_replace("<ul>","",$description_short);
            $description_short = trim($description_short);
            $description_short =  explode('<li>', $description_short);
            $description_short = array_filter($description_short, function($value) { return !is_null($value) && $value !== ''; });
            // die(var_dump($description_short));
            if(isset($description_short[0])){
                $description_short = array_chunk($description_short, count($description_short)/2);
            }
            $description_short_ = '<div class="col-xs-6">';
            // die(var_dump($description_short));
            if(isset($description_short[0])){
                foreach ($description_short[0] as $value) {
                    $description_short_ .= $value.'<br/>';
                }
            }
            $description_short_ .= '</div>';

            $description_short_ .= '<div class="col-xs-6">';
            // die(var_dump($description_short));
            if(isset($description_short[1])){
                foreach ($description_short[1] as $value) {
                    $description_short_ .= $value.'<br/>';
                }
            }
            $description_short_ .= '</div>';

            $description_ = str_replace("<li>","<br/>",$this->product->description);
            $description_ = str_replace("</li>","",$description_);
            $description_ = str_replace("</ul>","",$description_);
            $description_ = str_replace("<ul>","",$description_);
            $description_ = str_replace("<p>","",$description_);
            $description_ = str_replace("<div>","",$description_);
            $description_ = str_replace("</p>","",$description_);
            $description_ = str_replace("</div>","",$description_);
            // die(var_dump($description_));

            $accessories = Tools::getProductAccessories($id_product);
            if(isset($accessories[0]["reference"])){
                $accessories_ = "<div class='top'>
                                    <div class='short_'>Nr kat</div>
                                    <div class='long_'>Nazwa</div>
                                </div>";
                foreach ($accessories as $value) {
                    $accessories_ .= '<div class="bottom"><div class="short_">'.$value["reference"].'</div><div class="long_">'.$value["name"].'</div></div>';
                }
            ...
            <style>
                ...
                .container_sec{
                    clear: both;
                    margin-top: 20px;
                    border-bottom: 3px solid #ed192d !important;
                }
                .desc_content{
                    color: yellow;
                    clear: both;
            ...
            <html>
                ...
                <body>
                    <div class="container">
                        <div class="col-xs-5 border-right mt"><img src="'.$main_photo_url.'"></div>
                        <div class="col-xs-7 mt  fs22">
                            <div class="pb20 red" >'.$this->product->name.'</div>
                            <div class="pb20 mt20 fs16">Numer Katalogowy: '.$this->product->reference.'</div>
                            <div class="pb20 mt20">'.$qrcode.'</div>
                        </div>
                        <div class="container_sec">
                            <p class="header_">Opis produktu</p>
                        </div>
                        <div>
                            <p class="content_">'.$description_.'</p>
                        </div>

                        <div class="container_th container_sec">
                            <p class="header_">Dane techniczne</p>
                        </div>
                         <div>
                            <div class="content_"><br/>'.$description_short_.'</div>
                        </div>

                        <div class="container_sec">
                            <p class="header_" style="width: 220px !important;">Wyposażenie dodatkowe</p>
                        </div>
                        <div>
                            <div class="content_">
                            '.$accessories_.'
            ...
            $html = '<html>'.$head.'
                        <body>
                            <div class="container">
                            <div class="col-xs-5 border-right mt"><img src="'.$main_photo_url.'"></div>
                            <div class="col-xs-7 mt  fs22 red">'.$this->product->name.'</div>
                            <div class="col-xs-7 mt  fs22">Numer Katalogowy: '.$this->product->reference.'</div>
            ...
            // Tworzenie nowej instancji mPDF
            $mpdf = new \mPDF();
            
            // Tworzenie treści PDF
           // $html = '<h1>Przykładowy plik PDF</h1>
           // <p>To jest prosty przykład pliku PDF wygenerowanego za pomocą mPDF 5.7.</p>';
            
            // Dodaj treść do dokumentu PDF
            $mpdf->WriteHTML($html);
            
            // Wygeneruj plik PDF
            $mpdf->Output('przykladowy_plik.pdf', 'D'); // 'D' oznacza, że plik zostanie pobrany przez przeglądarkę
